...
function parseRss(xml: string): RssItem[] {
  ...
  for (const m of matches) {
    ...
    if (descMatch) item.description = stripHtml(decodeEntities(stripCdata(descMatch[1].trim())));
...
async function fetchFeed(source: FeedSource, signal: AbortSignal): Promise<NewsItem[]> {
  try {
    ...
    return toNewsItems(source, parseRss(xml));
  } catch {
    return [];
  }
}
  ...
  async fetch(filters: NewsFilters): Promise<NewsItem[]> {
    ...
    try {
      ...
      const allResults = await Promise.all(
        FEEDS.map((src) => fetchFeed(src, controller.signal)),
      );
