...
function buildReactPreview(
  ...
  const moduleMap: Record<string, { code: string; isTsx: boolean }> = {};
  ...
  const pluginsJson = JSON.stringify(pluginCandidates);
...
<body>
  ${bodyMarkup}
  ${hasNpmPackages ? "" : `<script src="${REACT_CDN}"></script>\n  <script src="${REACT_DOM_CDN}"></script>`}
  ...
  <script>
    ...
    function __require__(fromFile, importPath) {
      ...
      const transformedCode = __transform__(def.code, resolved, def.isTsx);
