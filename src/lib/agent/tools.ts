...
//
// Phase 11: path-traversal protection. read_file and write_file validate
// the requested path against the active project's file index BEFORE any
// filesystem operation. This blocks:
//   - absolute paths (e.g. "/etc/passwd")
//   - parent-traversal paths (e.g. "../../secret")
//   - cross-project access (the model cannot supply an arbitrary project id —
//     the active project comes from the DevWorkspace store, not from the
//     model's tool args)
...
/**
 * Validate a path is safe to operate on within the active project.
 *
 * Returns `{ ok: true, normalized }` on success, or
 * `{ ok: false, error }` if the path is rejected.
 *
 * Rejection rules:
 *   - Empty path
 *   - Absolute path (starts with "/" or a drive letter like "C:\\")
 *   - Contains ".." as a path segment (parent traversal)
 *   - Contains backslashes (normalize away — paths use forward slashes)
...
function validateProjectPath(
  ...
  // Normalize backslashes to forward slashes (Windows-style paths are
  // treated as project-relative after normalization).
  let p = rawPath.replace(/\\/g, "/");
  // Reject absolute paths.
  if (p.startsWith("/")) {
    return {
      ok: false,
      error: `Error: absolute paths are not allowed ("${rawPath}"). Use a project-relative path.`,
    };
  }
  // Reject Windows-style drive letters (C:/, D:/, etc.).
  if (/^[a-zA-Z]:\//.test(p)) {
    return {
      ok: false,
      error: `Error: drive-letter paths are not allowed ("${rawPath}"). Use a project-relative path.`,
    };
  }
  ...
  if (segments.some((seg) => seg === "..")) {
    return {
      ok: false,
      error: `Error: parent traversal ("..") is not allowed in paths.`,
    };
  }
  // Strip leading "./" if present (relative prefix is redundant).
  if (p.startsWith("./")) p = p.slice(2);
  // Reject control characters.
  if (/[\x00-\x1f]/.test(p)) {
    return {
      ok: false,
      error: `Error: path contains invalid control characters.`,
    };
  }
  ...
  return [
    ...
    },
    {
      ...
      async execute(args, ctx: ToolContext) {
        if (!ctx.project) return "No active project.";
        const validated = validateProjectPath(String(args.path ?? ""));
        if (!validated.ok) return validated.error;
        const path = validated.normalized;
        // Phase 11: confirm the file exists in the active project's index
        // before reading. This is the cross-project boundary — the model
        // cannot read files from any other project.
    ...
    },
    {
      ...
      async execute(args, ctx: ToolContext) {
        ...
        // Phase 11: before overwriting an existing file with a substantially
        // different content, create a recoverable project version snapshot
        // so the user can roll back. We only snapshot when the file already
        // exists AND the new content differs significantly (to avoid creating
        // hundreds of snapshots for tiny internal tool operations).
        const existing = ctx.project.files.find((f) => f.path === path);
        if (existing) {
          const oldContent = await ctx.readFile(path);
          if (oldContent !== undefined && oldContent !== content) {
            // Heuristic: "significant" = the diff is more than 20 chars or
            // the file is being replaced entirely. This is intentionally
            // conservative — we'd rather over-snapshot than lose data.
            const diffSize = Math.abs(oldContent.length - content.length);
            const isSignificant =
              diffSize > 20 ||
              oldContent.length === 0 ||
              content.length === 0;
            if (isSignificant && typeof ctx.createSnapshot === "function") {
              try {
                await ctx.createSnapshot(`Agent edit: ${path}`);
              } catch {
                // Non-fatal — the write should still proceed.
              }
            }
          }
        }
