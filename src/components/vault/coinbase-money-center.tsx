...
export function QuidaxMoneyCenter() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [accountResponse, addressResponse, transferResponse] = await Promise.all([
        fetch("/api/vault/quidax/accounts", { cache: "no-store" }),
        fetch("/api/vault/quidax/receive", { cache: "no-store" }),
        fetch("/api/vault/quidax/transfers", { cache: "no-store" }),
      ]);
      ...
      setError(loadError instanceof Error ? loadError.message : "Quidax could not be loaded.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function generateAddress() {
    ...
    setWorking(true);
    try {
      const response = await fetch("/api/vault/quidax/receive", {
        method: "POST",
  ...
  async function copyAddress(address: string) {
    ...
    toast({ title: "Address copied", description: "Verify it again in Quidax before transferring a large amount." });
  }
  ...
  async function createPreview() {
    ...
    setConfirmationText("");
    try {
      const response = await fetch("/api/vault/quidax/send/preview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function confirmSend() {
    ...
    setWorking(true);
    try {
      const response = await fetch("/api/vault/quidax/send/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      toast({ title: "Quidax transfer submitted", description: `Status: ${data.state}. Quidax transaction: ${data.providerTransactionId}` });
      setPreview(null);
      setPassword("");
      setConfirmationText("");
      setAmount("");
  ...
  if (loading) {
    return <VaultCard><VaultCardBody><div className="flex items-center gap-2 text-[11px] text-fg-muted"><RefreshCw className="h-3.5 w-3.5 animate-spin" />Loading Quidax wallets…</div></VaultCardBody></VaultCard>;
  }
  if (error || !settings) {
    return <VaultCard><VaultCardHeader title="Quidax Crypto" icon={<Bitcoin className="h-4 w-4" />} /><VaultCardBody><div className="rounded border border-amber-500/30 bg-amber-500/5 p-3 text-[11px] text-amber-200/80">{error ?? "Quidax is not connected."}</div></VaultCardBody></VaultCard>;
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 rounded-md border border-emerald-500/25 bg-emerald-500/5 px-3 py-2 themed">
        <div className="flex items-center gap-2 text-[11px] text-fg-muted"><ShieldCheck className="h-4 w-4 text-emerald-400" />Quidax owner connection · Receive enabled · Sends require password and exact confirmation</div>
        <Button variant="ghost" size="sm" onClick={() => void load()}><RefreshCw className="h-3.5 w-3.5" />Refresh</Button>
      </div>

      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <VaultCard>
          <VaultCardHeader title="Receive Crypto" subtitle="Generate a Quidax deposit address" icon={<ArrowDownToLine className="h-4 w-4" />} />
          <VaultCardBody>
            <div className="space-y-3">
              <FieldSelect label="Quidax wallet" value={receiveAccountId} onChange={setReceiveAccountId} options={accounts.filter((account) => account.type === "crypto").map((account) => ({ value: account.id, label: `${account.name} · ${account.available} ${account.asset}` }))} />
        ...
        <VaultCard>
          ...
          <VaultCardBody>
            {!settings.transfersEnabled && <div className="mb-3 rounded border border-amber-500/30 bg-amber-500/5 p-2.5 text-[10.5px] text-amber-200/80">Preview is available, but submission is locked. Set QUIDAX_TRANSFERS_ENABLED=true only after a receive/balance test succeeds.</div>}
            <div className="space-y-3">
              <FieldSelect label="From Quidax wallet" value={sendAccountId} onChange={(value) => { setSendAccountId(value); setPreview(null); }} options={accounts.filter((account) => account.type === "crypto").map((account) => ({ value: account.id, label: `${account.name} · ${account.available} ${account.asset}` }))} />
            ...
            {preview && (
              <div className="mt-4 space-y-3 rounded-md border border-red-500/35 bg-red-500/5 p-3">
                ...
                <Button onClick={() => void confirmSend()} disabled={working || !settings.transfersEnabled || confirmationText !== preview.confirmationText || !password} className="w-full">Confirm and send through Quidax</Button>
              </div>
            )}
          </VaultCardBody>
        </VaultCard>
