...
export function CinematicAuthLayout({ children }: CinematicAuthLayoutProps) {
  ...
  // WebGL-unavailable cases, we initialize to true via lazy useState
  // (no setState-in-effect needed).
  const [authCardVisible, setAuthCardVisible] = useState<boolean>(() => {
    if (typeof window === "undefined") return false;
    // Reduced-motion OR WebGL-unavailable → show the card immediately.
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return true;
    try {
      const testCanvas = document.createElement("canvas");
      const gl = testCanvas.getContext("webgl2") || testCanvas.getContext("webgl");
      if (!gl) return true; // WebGL unavailable → show immediately
    } catch { return true; }
    return false;
  });
