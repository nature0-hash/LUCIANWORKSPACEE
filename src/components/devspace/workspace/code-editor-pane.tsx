...
export function CodeEditorPane() {
  ...
  const saveCurrent = useCallback(async () => {
    ...
    await persistActive();
    // Settings → DevWorkspace → Preview → autoRefresh: when OFF, skip the
    // automatic preview refresh on save. The user clicks Refresh manually
    // in the preview toolbar to rebuild the preview.
    if (useSettingsStore.getState().devWorkspace.preview.autoRefresh) {
      refreshPreview();
    }
