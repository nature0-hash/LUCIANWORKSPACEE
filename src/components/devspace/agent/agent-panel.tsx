...
import { getProjectAgentProviderInfo } from "@/lib/agent/shared-provider";
...
export function AgentPanel({ compact = false }: AgentPanelProps) {
  ...
  // Phase 11: the provider label shows the actual shared AI provider
  // (e.g. "Google Gemini") rather than "No model configured" — the
  // shared provider adapter is always registered. We still surface a
  // warning if the server returns provider-not-configured (handled in
  // the submit path).
  const [providerLabel, setProviderLabel] = useState(() => {
    const info = getProjectAgentProviderInfo();
    return info.configured ? `${info.provider} · ${info.model}` : "No model";
  });
  const provider = useMemo(
    () => ({ label: providerLabel, configured: true }),
    [providerLabel],
  );
  ...
  const toolContext: ToolContext = useMemo(
    () => ({
      ...
      // Phase 11: create a recoverable project version snapshot. Used by
      // write_file before a significant overwrite. Uses the same IndexedDB
      // versions store as the manual History dialog — Agent edits and
      // manual edits share ONE project history.
      createSnapshot: async (label: string) => {
        ...
        try {
          ...
          const version = {
            ...
            label: label.slice(0, 80) || `Agent edit ${new Date().toISOString()}`,
            createdAt: Date.now(),
            files,
          ...
          // Non-fatal — snapshot failure should not block the write.
        }
      },
    }),
    [activeProject, loadFileContent, writeFile, refreshPreview, rescanActiveProject, getActiveProjectFiles],
  );
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Phase 11: refresh the provider label in case the user changed
      // the shared AI config in another tab / dialog.
      const info = getProjectAgentProviderInfo();
      setProviderLabel(info.configured ? `${info.provider} · ${info.model}` : "No model");
    } catch (err) {
...
function AgentSettingsDialog({
  ...
  // Phase 11: the dialog only needs label + configured + id for display.
  provider: { label: string; configured: boolean; id?: string };
}) {
  return (
      ...
      <DialogContent showCloseButton={false}>
        ...
        <div className="space-y-3 px-4 py-4 text-sm">
          ...
          <p className="text-xs text-fg-muted">
            The Project Agent is connected to LUCIAN&apos;s shared AI configuration
            (Phase 11). It uses the same provider as Economic Agent and Lilith
            unless overridden. To change the provider or model, open Settings
            → Lilith → Economic Agent Connection, or configure an interface
            override for the <code>dev-workspace-agent</code> interface.
          </p>
          <p className="text-xs text-fg-muted">
            API keys are stored as Vercel environment variables and read
            server-side by <code>/api/ai/chat</code> — they are never exposed
            to the browser or to the model.
          </p>
          <p className="text-xs text-fg-muted">
