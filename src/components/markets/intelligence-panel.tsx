...
function readStoredTab(): Tab {
  try {
    ...
    if (storedTab === "chat" || storedTab === "feed") return storedTab;
  } catch {
    /* storage unavailable */
  }
  return "feed";
...
export function IntelligencePanel({
  ...
  // Hydrate persisted state via lazy useState initializers (avoids the
  // React 19 set-state-in-effect rule).
  const [internalOpen, setInternalOpen] = useState<boolean>(readStoredOpen);
...
function ChatTab() {
  // Chat tab follows the ACTIVE chart pane — its symbol, timeframe, and
  // live bid/ask are pulled from the markets store's active-context
  // selector. This means switching panes instantly updates the chat
  // context strip and the system prompt sent to the model.
  ...
  const ticker = tickers.get(liveSymbol);
  const status: DataStatus = statusBySymbol.get(liveSymbol) ??
    (inst.marketOpen ? "live" : "setup-required");
  const liveBid = ticker?.bidPrice ?? livePrice ?? inst.bid;
  const liveAsk = ticker?.askPrice ?? livePrice ?? inst.ask;
  ...
  const buildContext = useCallback((): MarketChatContext => {
    return {
      ...
      bid: liveBid,
      ask: liveAsk,
      changePct: ticker?.priceChangePercent ?? inst.changePct,
      attachedNews: attachedNews.length > 0 ? attachedNews : undefined,
    };
  }, [inst, attachedNews, liveTimeframe, liveBid, liveAsk, ticker]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      {/* Market context strip — driven by the ACTIVE chart pane.
          Live price comes from the markets store; for non-crypto assets
          the catalog snapshot is shown and the status badge reads
          "Reference" so the trader knows the price isn't live. */}
      <div className="flex shrink-0 items-center gap-2 border-b border-line-muted px-3 py-1.5 themed">
        ...
        <span className="text-[10px] text-fg-muted">{liveTimeframe}</span>
        <span className="text-[9px] text-fg-muted">·</span>
        <span className="font-mono text-[10px] tabular-nums text-fg">
          {liveBid.toFixed(inst.pricePrecision)}
        </span>
        <ContextStatusBadge
          status={status}
          live={isSupportedCrypto(inst.symbol)}
        />
      </div>
      ...
      >
        {messages.length === 0 ? (
          <EmptyChatState
            ...
            onPickSuggestion={(q) => {
              setInput(q);
              // Send immediately so the suggestion feels like a one-click
              // conversation starter.
              setTimeout(() => {
                const text = q.trim();
                if (!text) return;
                // Build the message + send. We can't call handleSend
                // directly because it closures over `input` which hasn't
                // updated yet, so we replicate the minimal send path.
                const userMsg: MarketChatMessage = {
                  id: `msg_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
                  role: "user",
                  content: text,
                ...
                void fetch("/api/markets/chat", {
                  ...
                  .then((res) => res.json())
                  .then((data: { content: string; fromModel: boolean }) => {
                    setMessages((prev) => [
                      ...prev,
                      {
                        id: `msg_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
                  ...
                  .catch(() => {
                    setMessages((prev) => [
                      ...prev,
                      {
                        id: `msg_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
                        role: "assistant",
                        content: "Failed to reach the Markets AI service. Please try again in a moment.",
                        timestamp: Date.now(),
                        fromModel: false,
                      },
                    ]);
      ...
      <div className="shrink-0 border-t border-line-muted p-2">
        ...
        <div className="mt-1 flex items-center justify-between">
          {/* Phase 3: the "Context" + "Model" composer buttons were removed
              because they had no onClick handler. The chat context (symbol,
              timeframe, bid/ask, attached news) is already built into every
              outgoing message via `buildContext()`, and the model is picked
              server-side by `/api/markets/chat`. Adding a fake affordance
              for either would mislead the user. */}
          <div className="text-[9px] text-fg-faint">
            Context: {inst.symbol} · {liveTimeframe}
          </div>
          <button
            type="button"
            onClick={handleSend}
      ...
      <div className="mt-2 space-y-1">
        ...
        {[
          ...
          <button
            ...
            className="block w-full rounded border border-line-muted bg-surface-2 px-2 py-1 text-left text-[10px] text-fg-muted transition-colors hover:bg-hover hover:text-fg"
          >
            {q}
          </button>
        ))}
...
/* ── Context status badge for the Chat tab's context strip ──
   Mirrors OrderDetailsPanel's OrderStatusPill — a small colored pill that
   tells the user at a glance whether the chat is grounded in LIVE provider
   data, REFERENCE catalog prices, or UNAVAILABLE live data. */
function ContextStatusBadge({
  ...
  if (!live) {
    return (
      <span
        title="No live provider — using reference catalog prices"
        className="rounded border border-line-muted px-1 py-0.5 text-[8px] font-bold uppercase tracking-wide text-fg-faint"
      >
        Reference
      </span>
    );
  ...
  return (
    <span
      title={`Live provider status: ${label}`}
