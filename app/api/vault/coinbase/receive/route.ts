...
import { createReceiveAddress, listReceiveAddresses } from "@/lib/coinbase/transfers";
...
export async function GET() {
  ...
  try {
    const addresses = await listReceiveAddresses(result);
    return NextResponse.json({ addresses: addresses.map((row) => ({
      id: row.id, asset: row.asset, network: row.network, address: row.address,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const address = await createReceiveAddress(result, input);
